...
Check that the results from running `./e2etest` are the same
between the most recent run, and the last run

NOTE: This is a terrible way to test things. I'm only doing it like this
during refactoring to set up a "guardrail", as I try to despaghettify the code.
...
class E2ETest(unittest.TestCase):
    ...
    def test_e2e(self):
        ...
        take = [0, -1]  # take these array indexes to compare

        lineages = []
        # consider the most recent two runs
        take_folders = [results_dirs[i] for i in take]
        all_paths = [os.path.join(t, "seed-0/lineages.pkl")
        ...
        for seconds in take_folders:
            # pull the seconds out of "raw_results/12345"
            t = time.strftime('%Y-%m-%d %H:%M:%S',
                              time.localtime(int(re.findall('[0-9]+', seconds)[0])))
        ...
        # compare each lineage against the first
