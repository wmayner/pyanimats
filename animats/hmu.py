...
class Individual:
    ...
    def initializeClass(cls, experiment):
        '''
        loads static vars onto the Individual class
        '''
        # i = Individual  # simple shorthand
        ...
        if ("init_genome_path" in experiment) and \
           (experiment['init_genome_path'] != ""):
            with open(experiment['init_genome_path'], 'rb') as f:
                ...
                cls.INIT_GENOME = lineages[0][0].genome
        else:
            cls.INIT_GENOME = \
        ...
        cls.num_sensor_states = 2**experiment['num_sensors']
        cls.num_motor_states = 2**experiment['num_motors']

        # cls.sensor_motor_states = (lambda nss, nms, _bitlist, experiment: [
        ...
        cls.sensor_motor_states = [
            ((i, j), (_bitlist(i, experiment['num_sensors']) +
                      _bitlist(j, experiment['num_motors'])))
            for i in range(cls.num_sensor_states)
            for j in range(cls.num_motor_states)
        ]

        cls.nat_to_bit_conversion_factor = 1 / math.log(2)
